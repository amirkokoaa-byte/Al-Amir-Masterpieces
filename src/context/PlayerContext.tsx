...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (currentStation && audioRef.current) {
      ...
      // Only proxy radiojar or non-https streams through local server if running locally
      if ((src.includes('radiojar.com') || src.startsWith('http://')) && window.location.hostname === 'localhost') {
    ...
    const handleError = async () => {
      ...
      if (currentStation?.stationuuid === 'ertu-quran-egypt' && !isFallback) {
         try {
           ...
           // Fallback to Qurango Tarteel stream if the primary misrquran stream fails
           const backupUrl = 'https://backup.qurango.net/radio/tarteel';
